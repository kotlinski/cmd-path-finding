...
/*
  public void testKShortestPathCompleteGraph4() {
    ...
    assertEquals("[[(vS : v1), (v1 : v3)], [(vS : v2), (v2 : v3)]," + " [(vS : v2), (v1 : v2), (v1 : v3)], "
        + "[(vS : v1), (v1 : v2), (v2 : v3)], " + "[(vS : v3)]]", pathElements.toString());
    ...
    assertEquals(Arrays.asList(new Object[] { graph.eS1, graph.e13 }), pathElement.getEdgeList());
  }
  ...
  public void testPicture1Graph() {
    ...
    KShortestPaths pathFinder = new KShortestPaths(picture1Graph, "vS", maxSize);

    // assertEquals(2, pathFinder.getPaths("v5").size());

    List pathElements = pathFinder.getPaths("v5");
    GraphPath pathElement = (GraphPath) pathElements.get(0);
    assertEquals(Arrays.asList(new Object[] { picture1Graph.eS1, picture1Graph.e15 }), pathElement.getEdgeList());

    List vertices = GraphOperator.getPathVertexList(pathElement);
    assertEquals(Arrays.asList(new Object[] { "vS", "v1", "v5" }), vertices);

    pathElement = (GraphPath) pathElements.get(1);
    assertEquals(Arrays.asList(new Object[] { picture1Graph.eS2, picture1Graph.e25 }), pathElement.getEdgeList());

    vertices = GraphOperator.getPathVertexList(pathElement);
  ...
  private void verifyShortestPathsWeightsWithMaxSizeIncreases(Graph graph) {
    ...
    for (Iterator sourceIterator = graph.vertexSet().iterator(); sourceIterator.hasNext();) {
      ...
      for (Iterator targetIterator = graph.vertexSet().iterator(); targetIterator.hasNext();) {
        ...
        if (targetVertex != sourceVertex) {
          KShortestPaths pathFinder = new KShortestPaths(graph, sourceVertex, 1);
          List<GraphPath> prevPathElementsResults = pathFinder.getPaths(targetVertex);
          ...
          for (int maxSize = 2; maxSize < maxSizeLimit; maxSize++) {
            ...
            List<GraphPath> pathElementsResults = pathFinder.getPaths(targetVertex);
  ...
  *//**
   ...
   * increases.
   *
   * @param prevPathElementsResults
   *          results obtained with a max-size argument equal to <code>k</code>
   * @param pathElementsResults
   *          results obtained with a max-size argument equal to
   *          <code>k+1</code>
   *//*
  private void verifyWeightsConsistency(List<GraphPath> prevPathElementsResults, List<GraphPath> pathElementsResults) {
    for (int i = 0; i < prevPathElementsResults.size(); i++) {
      GraphPath pathElementResult = (GraphPath) pathElementsResults.get(i);
  ...
  *//**
   * Currently disabled since it takes more than a few seconds to run. Also need
   * to actually check the output instead of printing.
   *
   * @see <a href=
   *      "http://jgrapht-users.107614.n3.nabble.com/quot-graph-must-contain-the-start-vertex-quot-when-running-KShortestPaths-td4024797.html">
   *      bug description</a>.
   *//*
  public void _testIllegalArgumentExceptionGraphNotThrown() throws Exception {
    SimpleWeightedGraph<String, Edge> graph = new SimpleWeightedGraph<String, Edge>(
        Edge.class);
    ...
    Edge src = graph.getEdge("M013", "M014");

    KShortestPaths<String, Edge> kPaths = new KShortestPaths<String, Edge>(graph,
        graph.getEdgeSource(src), 5);
    List<GraphPath<String, Edge>> paths = null;

    try {
      paths = kPaths.getPaths(graph.getEdgeTarget(src));
      for (GraphPath<String, Edge> path : paths) {
        for (Edge edge : path.getEdgeList()) {
          System.out.print("<" + graph.getEdgeSource(edge) + "\t" + graph.getEdgeTarget(edge) + "\t" + edge + ">\t");
        }
